...
for n = 1 : nJobs
    ...
    beam_plane_std_dev = JobFile.Parameters.BeamPlaneStdDev;
    ...
    for s = 1 : nSets
        ...
        if reSeed
            ...
            if parallel_processing
                ...
                parfor k = 1 : imagesPerSet % Parallel loop over all the images
                    ...
                    % Generate the particle image pair.
                    [image_01, image_02] = generateImagePair_3D_mc( ...
                        region_height, region_width, region_depth, ...
                        particle_diameter_mean, ...
                        particle_diameter_std_list(k), ...
                        concentrations(k), beam_plane_std_dev, tforms(:, :, k), run_compiled);
                    ...
                    switch image_dimensionality
                        ...
                        % to the data matrix
                        imageMatrix1_2D(:, :, k) = sum(image_01, 3) + ...
                            cast(noiseMatrix1(:, :, k), ...
                            'like', image_01);

                         % Save z-projection of second volume
                         % to data matrix
                        imageMatrix2_2D(:, :, k) = sum(image_02, 3) + ...
                            cast(noiseMatrix2(:, :, k), ...
                            'like', image_02);
                        ...
                        otherwise
                            ...
                            imageMatrix1_3D(:, :, :, k) = image_01 + ...
                                cast(noiseMatrix1(:, :, :, k), ...
                                'like', image_01);

                             % Save second image to data matrix
                            imageMatrix2_3D(:, :, :, k) = image_02 + ...
                                cast(noiseMatrix2(:, :, :, k), ...
                                'like', image_02);
                ...
                for k = 1 : imagesPerSet % Parallel loop over all the images
                    ...
                    switch image_dimensionality
                        ...
                        % Save z-projection of first volume 
                        % to the data matrix
                        imageMatrix1_2D(:, :, k) = cast(sum(image_01, 3) + ...
                            noiseMatrix1(:, :, k), ...
                            'like', image_01);
        ...
        % This statement is accessed when reSeed == 0;
           ...
           imageMatrix1 = imageMatrix1 + cast(noiseMatrix1, 'uint16');
